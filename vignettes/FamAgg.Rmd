...
Section [3](#org7e444ba)).  Upon data set creation the kinship matrix (i.e. a
...
# Importing and exporting pedigree data<a id="org7e444ba"></a>
...
index* method [@Hill:1980tz] (Section [4.1](#orgbc1ab19)), then we estimate the
...
[4.2](#org156844d)) and apply our *kinship sum test* to identify affected individuals
exhibiting a higher relationship to other affected individuals than what would
be expected by chance (Section [4.3](#orge5029ab)). Subsequently, we apply our
*kinship group test* (Section [4.4](#org028cffe)) that allows to identify highly
clustered affected individuals within families.

In Section [4.5](#org3ef3a2a) we apply the *probability test* based on the method from
...
## *Genealogical index of familiality* <a id="orgbc1ab19"></a>
...
## *Familial incidence rate* (FIR)<a id="org156844d"></a>
...
## *Kinship sum test* <a id="orge5029ab"></a>
...
## *Kinship group test* <a id="org028cffe"></a>
...
## Exact inference for family disease clusters (*probability test*)<a id="org3ef3a2a"></a>
...
```{r  probtest-1, message=FALSE, eval = !isWindows }
...
```{r  probtest-2, message=FALSE, eval = !isWindows }
...
```{r  probtest-3, mbreast-prob-clique-1, message=FALSE, warning=FALSE, fig.align='center', eval = !isWindows }
